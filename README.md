...
A sandboxed Debian VM with no direct internet access. All traffic is forced through a host-side [mitmproxy](https://mitmproxy.org/) that enforces an allowlist, giving full visibility and control over what the guest can reach. Runs on macOS (socket\_vmnet + HVF) and Linux (QEMU user networking + TCG/KVM).
